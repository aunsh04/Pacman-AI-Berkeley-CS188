...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
        	allPossible[self.getJailPosition()]=1     #special case
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        c = util.Counter()
        for p in self.legalPositions:
                g=self.setGhostPosition(gameState, p)
        	newPosDist = self.getPositionDistribution(g) #distribution over new positions for the ghost
        	for newPos, prob in newPosDist.items():
        		c[newPos] = c[newPos]+ (prob * self.beliefs[p])  
        self.beliefs=c
        return self.beliefs
        util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        s=[]
        pnumber = self.numParticles
        even = pnumber/len(self.legalPositions)
        for i in range(even):
            for p in self.legalPositions:
                s.append(p)
        self.particles=s
    ...
    def observe(self, observation, gameState):
        ...
        pnumber=self.numParticles
        allPossible = util.Counter()
        b = self.getBeliefDistribution()
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if noisyDistance is None:            # ghost captured
        	s=[]
        	for i in range(pnumber):
                    s.append(self.getJailPosition())
                allPossible[self.getJailPosition()]=1
                self.particles=s
            if emissionModel[trueDistance] > 0:
                allPossible[p] = emissionModel[trueDistance] * b[p]
        allPossible.normalize()
        if allPossible.totalCount()==0:         # 0 weight
            self.initializeUniformly(gameState)
        else:
            s=[]
            for i in range(pnumber):
                s.append(util.sample(allPossible))    #resample
            self.particles=s
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
                g=self.setGhostPosition(gameState, p)
        	newPosDist = self.getPositionDistribution(g)
                s.append(util.sample(newPosDist))
        self.particles=s                    
        return self.particles
        util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        s=[]
        l= (list (itertools.product(self.legalPositions,repeat=self.numGhosts)))
        random.shuffle(l)
        pnumber=self.numParticles
        while pnumber!=0:
            for p in l:
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i]==None:
                    self.particles[part]=self.getParticleWithGhostInJail(self.particles[part],i)
                else:
                    trueDistance = util.manhattanDistance(self.particles[part][i],pacmanPosition)
                    e=e*emissionModels[i][trueDistance]
            allPossible[self.particles[part]]+=e
            part+=1
        if allPossible.totalCount()==0:
            ...
            for p in self.particles:
                for i in range(self.numGhosts):
                    if noisyDistances[i]==None:
                        self.particles[it]=self.getParticleWithGhostInJail(self.particles[it],i)
                it+=1
        else:
            allPossible.normalize()
            self.particles=[]
