...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
   	...
   	if top>bottom:
		bright2=True
    features["top-bottom"] =bright2           #top-bottom brightness
    ...
    for x in range(DIGIT_DATUM_WIDTH/2, DIGIT_DATUM_WIDTH):
   	...
   	if left>right:
		bright1=True
    features["left-right"] =bright1                   #left-right brightness
    
    a=0.0
    a=DIGIT_DATUM_HEIGHT/DIGIT_DATUM_WIDTH
    features["aspectratio"] = a
    ...
    def find_loop(x,y):
        ...
        if (x,y) in marked.keys():
            return True

        marked[(x,y)]=True
        return find_loop(x-1,y) and find_loop(x+1,y) and find_loop(x,y-1) and find_loop(x,y+1)
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    s = state.generatePacmanSuccessor(action)
    newPos = s.getPacmanPosition()
    newFood = s.getFood()
    newGhostPos = s.getGhostPositions()
    ...
    for g in newGhostPos:
        ...
        if d<minG:
        	minG=d
	features['closest ghost'] = minG       #SuicideAgent
    
    minF=9999
    for f in newFood.asList():
    	f=util.manhattanDistance(newPos,f) 
        if f<minF:									#ContestAgent		
        	minF=f
    features['closest food'] = (1.0/(minF+0.1))
    return features
    util.raiseNotDefined()
    return features
